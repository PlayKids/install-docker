	...
	case "$lsb_dist" in
		ubuntu|debian)
			...
			(
				...
				$sh_c 'apt-get update'
				$sh_c "apt-get install -y -q docker-ce=$(apt-cache madison docker-ce | grep ${docker_version} | head -n 1 | cut -d ' ' -f 4) docker-ce-cli=$(apt-cache madison docker-ce-cli | grep ${docker_version} | head -n 1 | cut -d ' ' -f 3)"
			)
			echo_docker_as_nonroot
			exit 0
			;;
